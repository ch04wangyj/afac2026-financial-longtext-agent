...
def chunk_document(document: Document, pages: list[PageText], max_chars: int = 1800) -> list[Chunk]:
    ...
    chunks: list[Chunk] = []
    for page in pages:
        parts = _split_page(page.text, document.domain, max_chars=max_chars)
        for idx, part in enumerate(parts):
            part = normalize_text(part)
            if len(part) < 20:
                continue
            section = _infer_section(part)
            clause_id = _infer_clause(part)
            chunk_id = _chunk_id(document.doc_id, page.page, idx, part)
            chunks.append(
                Chunk(
                    chunk_id=chunk_id,
                    ...
                    clause_id=clause_id,
                    text=part,
                    tables=page.tables if idx == 0 else [],
                    numbers=extract_numbers(part),
                    dates=extract_dates(part),
                    metadata={"title": document.title, "path": document.path},
                )
            )
    return chunks
