"""脚本 26：在 V6 官网基线上合并指定的 V7 单变量复核组。"""
...
def main() -> None:
    parser = argparse.ArgumentParser(description="生成 V7 针对性复核候选。")
    parser.add_argument(
        "--baseline",
        type=Path,
        default=ROOT / "outputs" / "releases" / "v6" / "answer_results.jsonl",
    )
    parser.add_argument(
        "--config",
        type=Path,
        default=ROOT / "configs" / "v7_targeted_reviews.json",
    )
    parser.add_argument(
        "--include-group",
        action="append",
    ...
    merged = merge_candidate_with_baseline(
        ...
        reviews,
        baseline_label="v6_official_83.33",
        candidate_label="v7_targeted_review",
    )
    ...
    print(f"V6 Token: {baseline_tokens:,}")
