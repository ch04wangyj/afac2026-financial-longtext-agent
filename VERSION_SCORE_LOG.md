...
- **最佳得分**：**68.6873**
- **相对 V13 提升**：`+2.4281`
- **准确率说明**：按本地留存 Token `312,541` 与官方公式反推为 **70 / 100**
...
## V16 candidate｜2026-06-28 结构导航、文档绑定与逐项真值组装
- **得分**：待官网提交验证，不把本地推算写成真实得分。
- **目标**：当前 `315,727` Token 下，正确数达到 `82/100` 时预计综合分约 `80.45`。
- **相对 V14 答案变化**：14 题；每题均在 `configs/v16_structure_reviews.json` 记录原文依据。
- **关键改动**：
  - 在 V14 平铺 BM25F 旁路增加 PageIndex-lite 页面/章节导航，导航只增补候选，不删除全局 BM25 命中。
  - 为保险数字文档、财报公司和研报主题增加业务别名；选项点名产品或 `fc_text_00N` 时限定文档范围。
  - Qwen 对每个选项输出题干语义下的 `true/false/uncertain`，多选答案由已验证真值确定性组装。
  - 修复“解释括号被当成正确事实”“不同保险产品条款串用”“违约赔偿公式关系词漏召回”等问题。
  - 候选全量 54 题变化中仅接受 14 个直接证据闭环，其余全部回退 V14。
- **Token**：`315,727`，相对 V14 增加 `3,186`（约 `1.0%`）。
- **开发集**：`3/3`；100 题覆盖、答案格式、逐题 Token 和汇总均通过。
- **提交产物**：`outputs/submissions/v16_structure_target_20260628/answer.csv`。
